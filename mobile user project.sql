...
DROP TABLE IF EXISTS gender_age_train CASCADE;
...
DROP TABLE IF EXISTS new_app_labels;
...
SELECT device_id, phone_brand, device_model, COUNT(*) FROM phone_brand 
Group BY (device_id, phone_brand, device_model) HAVING COUNT(*) > 1 ORDER  BY device_id;
...
SELECT app_id, label_id, COUNT(*) FROM app_labels 
Group BY (app_id, label_id) HAVING COUNT(*) > 1 ORDER  BY app_id;
...
SELECT phone_brand, COUNT(*) FROM new_phone_brand 
GROUP BY phone_brand ORDER BY COUNT(*) DESC LIMIT 10;
...
GROUP BY n.gender, n.phone_brand ORDER BY n.gender, COUNT(*) DESC LIMIT 10;
...
ROW_NUMBER() OVER (PARTITION BY gender ORDER BY COUNT(*) DESC) AS row_num_rank FROM phone_train GROUP BY gender, phone_brand
...
SELECT app_id, COUNT(*) FROM app_events WHERE is_installed = '1' AND is_active = '1'
